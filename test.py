...
def get_faces():
    df1_face = pd.read_csv('https://glassgow.s3.amazonaws.com/csv/face_url%2B2%2B(1)%2B(4)%2B(2).csv')
    df2_face = df1_face[['face1','face2']]
    df3_face = df2_face.sample(n=3)
    face1_list = []
    face2_list = []
    face_list = []
...
def face_training(number):
    if number == 1:
        ...
        df_face = pd.read_csv('https://glassgow.s3.us-east-2.amazonaws.com/csv_folder/44_short_gfmt_training.csv')
    elif number == 4:
        df_face = pd.read_csv('https://glassgow.s3.us-east-2.amazonaws.com/csv_folder/51_short_gfmt_training.csv')
    elif number == 5:
        df_face = pd.read_csv('https://glassgow.s3.us-east-2.amazonaws.com/csv_folder/58_short_gfmt_training.csv')
    elif number == 6:
        df_face = pd.read_csv('https://glassgow.s3.us-east-2.amazonaws.com/csv_folder/65_short_gfmt_training.csv')
    elif number == 7:
        df_face = pd.read_csv('https://glassgow.s3.us-east-2.amazonaws.com/csv/face_url%2B2%2B(1)%2B(4)%2B(2).csv')
    else:
        return None
    df_face1 = df_face[['face1','face2']]
    df2_face = df_face1.sample(n=3)
    train_face_list1 = []
    train_face_list2 = []
